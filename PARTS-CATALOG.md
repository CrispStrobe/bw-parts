...
## MCU Boards (6 kinds + 1 declined)

| # | Kind slug | Name EN / DE | Engine | Art | Confidence |
|---|-----------|-------------|--------|-----|------------|
| 93 | `arduino_uno` | Arduino Uno R3 / Arduino Uno R3 | modeled (avr8js) | done | `standard`: ATmega328P |
| 94 | `arduino_nano` | Arduino Nano / Arduino Nano | modeled (avr8js) | done | `standard`: ATmega328P |
| 95 | `attiny85` | ATtiny / ATtiny | modeled (avr8js, a7fef9a) | dip-gen | **unverified**: probably ATtiny85 |
| 96 | `microbit` | micro:bit / micro:bit | drawable-only | done | **unverified**: v1 vs v2 |
| 97 | `stc_mcu` | STC12/STC15 MCU / STC-MCU | modeled (emu8051) | dip-gen | unique to bw |
| 98 | `pi_pico` | Raspberry Pi Pico / Raspberry Pi Pico | drawable-only | done | `standard`: RP2040 |
...
| # | Kind slug | Name EN / DE | Terminals | Engine | Art |
|---|-----------|-------------|-----------|--------|-----|
| 99 | `multimeter` | Multimeter / Multimeter | probe_a, probe_b | modeled | done |
| 100 | `power_supply` | Power Supply / Netzteil | pos, neg | modeled | done |
| 101 | `function_gen` | Function Generator / Funktionsgenerator | out, gnd | modeled | done |
| 102 | `oscilloscope` | Oscilloscope / Oszilloskop | ch1, ch2, gnd | modeled | done |
...
|---|-----------|-------------|--------|-----|
| 103 | `breadboard_full` | Breadboard Standard / Steckbrett | drawable-only | done |
| 104 | `breadboard_half` | Breadboard Small / Steckbrett Klein | drawable-only | done |
| 105 | `breadboard_mini` | Breadboard Mini / Steckbrett Mini | drawable-only | done |
| 106 | `header` | 8-Pin Header / 8-Pin-Stiftleiste | drawable-only | done |
| 107 | `usb_a` | USB-A Connector / USB-A-Stecker | drawable-only | done |
| 108 | `microbit_breakout` | micro:bit with Breakout / micro:bit mit Breakout | drawable-only | done |
| 109 | `pololu_motor_ctrl` | Pololu Motor Controller / Pololu-Motorsteuerung | drawable-only | done |
...
| # | Kind slug | Notes | Art |
|---|-----------|-------|-----|
| 110 | `switch` | SPST toggle (engine has it alongside slide_switch) | done |
| 111 | `ntc` | NTC thermistor (engine models it; TMP36 is analog alternative) | done |
| 112 | `eeprom` | I2C EEPROM (24LC256-class) | done |
| 113 | `temp_sensor` | DS18B20 1-wire temp sensor (different from TMP36) | done |
| 114 | `led_matrix` | 8x8 LED matrix (standalone, no MAX7219) | done |
| 115 | `led_cube` | LED cube (unique to bw) | done |
| 116 | `mcu` | Generic MCU (base for arch-specific boards) | done |
| 117 | `vcc` | VCC supply rail | done |
| 118 | `gnd` | Ground reference | done |
| 119 | `vsource` | Ideal voltage source | done |
| 120 | `isource` | Ideal current source | done |
| 121 | `fuse` | Fuse (glass cartridge) | done |
| 122 | `solenoid` | Solenoid (electromagnetic actuator) | done |
| 123 | `stepper` | Stepper motor (4-wire bipolar) | done |
| 124 | `ir_transmitter` | IR LED transmitter | done |
...
| Multi-arch boards | 2 | 2 |
| Engine-only extras | 15 | 15 |
| Declined | 1 | — |
| **Total** | **124 + 1 declined** | **124** |
